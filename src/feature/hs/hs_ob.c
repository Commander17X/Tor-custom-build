 ...
 * address with the ".sn" extension, find the public key and put it in
 * pkey_out.
 *
...
get_onion_public_key(const char *value, ed25519_public_key_t *pkey_out)
{
  ...
  if (strcmpend(value, ".sn")) {
    /* Not a .sn extension, bad format. */
    return false;
  }

  /* Length validation. The -1 is because sizeof() counts the NUL byte. */
  if (strlen(value) >
      (HS_SERVICE_ADDR_LEN_BASE32 + sizeof(".sn") - 1)) {
    /* Too long, bad format. */
    return false;
  }

  /* We don't want the .sn so we add 2 because size - 1 is copied with
   * strlcpy() in order to accommodate the NUL byte and sizeof() counts the NUL
   * byte so we need to remove them from the equation. */
  strlcpy(address, value, strlen(value) - sizeof(".sn") + 2);
